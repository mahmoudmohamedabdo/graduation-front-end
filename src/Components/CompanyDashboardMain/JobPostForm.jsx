...
export default function JobPostForm({ refreshJobs }) {
  ...
  const validateTaskForm = () => {
    ...
    if (!taskData.estimatedHours) {
      ...
      !/^\d+$/.test(taskData.estimatedHours) ||
      parseInt(taskData.estimatedHours) <= 0 ||
      parseInt(taskData.estimatedHours) > 1000
    ) {
      fieldErrors.estimatedHours = 'Estimated Hours must be a positive integer between 1 and 1000';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        const endpoint = isEditMode ? `http://fit4job.runasp.net/api/Jobs/${id}` : 'http://fit4job.runasp.net/api/Jobs';
        ...
        if (result.success) {
          setSuccess(true);
          const jobId = isEditMode ? id : result.data?.id;
          if (!jobId) {
            throw new Error('Job ID not returned from server');
          }
          setTaskData((prev) => ({ ...prev, jobId: parseInt(jobId) }));
          if (refreshJobs) {
            refreshJobs();
          }
          if (!isEditMode) {
          ...
          } else if (assessmentOption === 'Add Quiz' || assessmentOption === 'Add Quiz and Task') {
            navigate(`/add-exam/${jobId}`);
          } else {
    ...
    if (validateTaskForm()) {
      try {
        ...
        if (result.success) {
          ...
          if (assessmentOption === 'Add Quiz and Task') {
            navigate(`/add-exam/${taskData.jobId}`);
          } else {
            navigate('/companydashboard');
          }
...
4 sm:mb-6">
          ...
          {!showTaskForm ? (
            ...
            <form onSubmit={handleTaskSubmit} className="space-y-3 sm:space-y-4 md:space-y-5">
              ...
              {taskSuccess && <div className="text-green-500 mb-2 sm:mb-4">Task created successfully!</div>}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
                <div>
                  ...
                  <input
                    ...
                    required
                  />
                  {taskErrors.deadline && <div className="text-red-500 text-xs sm:text-sm mt-1">{taskErrors.deadline}</div>}
                </div>
                <div>
                  <label className="block text-xs sm:text-sm md:text-base font-medium text-gray-700 mb-1 sm:mb-2">
                    Estimated Hours *
