...
export default function JopDetails() {
  ...
  const handleApply = async () => {
    ...
    if (!userId || !jobId) {
      setApplyMessage("User ID or Job ID missing");
      return;
    }
    ...
    try {
      ...
      if (response.ok && result.success) {
        setApplyMessage("✅ You have successfully applied for this job!");
      } else {
        setApplyMessage(`❌ Failed to apply: ${result.message || "Unknown error"}`);
      }
    } catch (error) {
      setApplyMessage(`❌ Error occurred: ${error.message}`);
    } finally {
  ...
  return (
    <div className="container px-4 mx-auto w-sm-full">
      ...
      {/* Apply Now */}
      <div className="flex justify-end mb-10 flex-col items-end gap-3">
        {applyMessage && (
          <div className="text-sm text-gray-700 bg-gray-100 p-3 rounded-md w-full max-w-md">
            {applyMessage}
          </div>
        )}
        <button
          onClick={handleApply}
          className="bg-[#16A34A] text-white font-medium py-3 px-8 rounded-full shadow-lg transition duration-300 disabled:opacity-50"
